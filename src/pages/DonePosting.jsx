...
const DonePosting = () => {
    ...
    const filteredPosts = posts.filter(post => {
        ...
        return matchesSearch && matchesSocial && matchesPlatform && matchesPostType;
    }).sort((a, b) => {
    ...
    return (
        <div className="p-10 font-sans bg-light-bg min-h-screen text-slate-900">
            ...
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-slate-900 text-[32px] font-extrabold m-0">Done Posting</h1>
                    <p className="text-slate-500 mt-1 mb-0">View your published content</p>
                </div>
                <button
            ...
            {loading ? (
                ...
                <div className={viewMode === 'grid' ? 'grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] gap-6' : 'flex flex-col gap-3'}>
                    {filteredPosts.map(post =>
                        viewMode === 'grid' ? (
                            <PostCard key={post.id} post={post} userRole={userRole} currentUserId={userId} orgId={orgId} isSelected={selectedPosts.includes(post.id)} onSelect={() => toggleSelectPost(post.id)} />
                        ) : (
                            <PostListRow key={post.id} post={post} userRole={userRole} currentUserId={userId} orgId={orgId} isSelected={selectedPosts.includes(post.id)} onSelect={() => toggleSelectPost(post.id)} />
                        )
                    )}
                </div>
            ) : (
                <div className="text-center py-24 bg-white rounded-3xl border-2 border-dashed border-slate-200">
                    <p className="text-slate-500 font-semibold">No published posts found.</p>
                </div>
            )}
